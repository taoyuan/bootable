...
function Initializer(options) {
  options = options || {};
  this._phases = [];
  this._eventable = options.eventable !== false;
}
...
Initializer.prototype.run = function(cb, thisArg) {
  var phases = this._phases
      , idx = 0
      , eventable = !!this._eventable && thisArg && !!thisArg.emit
      , name;

  function next(err) {
    if (err) { return cb(err); }

    if (eventable && name) thisArg.emit(name + ':after', thisArg);
    ...
    try {
      if (typeof phase == 'object') {
        ...
        name = eventable && phase.constructor && phase.constructor.name;
        if (eventable && name) thisArg.emit(name + ':before', thisArg);
        phase.boot(next);
      } else {
        debug('%s', phase.name || 'anonymous');
        name = eventable && phase.name;
        if (eventable && name) thisArg.emit(name + ':before', thisArg);
