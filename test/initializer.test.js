...
describe('Initializer', function () {
  ...
  describe('eventable', function () {
    describe('that fire events', function () {
      var ee = new EventEmitter()
        , initializer = new Initializer()
        , order = [];

      initializer.phase(function something() {
        order.push("something");
      });

      ee.on('something:before', function () {
        order.push('something:before');
      });

      ee.on('something:after', function () {
        order.push('something:after');
      });
      ...
      it('should fire before and after events', function () {
        expect(order).to.have.length(3);
        expect(order).to.eql(['something:before', 'something', 'something:after'])
      });
    });
  });
